        ...
        for identifier in self.IDENTIFIERS:
            ...
            for ticket_number in range(1, identifier + 1):
                Ticket.objects.create(
                    title='Unassigned Ticket %d in Category %d' % (ticket_number, identifier),
                    queue=queue,
                )
                Ticket.objects.create(
                    title='Ticket %d in Category %d Assigned to User_%d' % (ticket_number, identifier, identifier),
                    queue=queue,
                    assigned_to=user,
                )
    ...
    def test_ticket_list_per_queue_user_restrictions(self):
        ...
        for identifier in self.IDENTIFIERS:
            ...
            self.assertEqual(
                ...
                1,
                'Category choices were not properly limited by queue membership'
            )
            self.assertEqual(
                response.context['queue_choices'][0],
                Queue.objects.get(title="Category %d" % identifier),
                'Category choices were not properly limited by queue membership'
            )
    ...
    def test_ticket_reports_per_queue_user_restrictions(self):
        ...
        for identifier in self.IDENTIFIERS:
            ...
            self.assertEqual(
                ...
                2,
                'Categories in report were not properly limited by queue membership'
            )
        ...
        self.assertEqual(
            ...
            3,
            'Categories in report were improperly limited by category membership for a superuser'
        )
        # Superuser should see the total ticket count of three tickets
        self.assertEqual(
            sum([sum(user_tickets[1:]) for user_tickets in response.context['data']]),
            6,
            'Tickets in report were improperly limited by category membership for a superuser'
        )
        self.assertEqual(
            len(response.context['headings']),
            3,
            'Category choices were improperly limited by category membership for a superuser'
        )
