...
"""
This migration script adds and removes models related to the Helpdesk system.

It includes creation and deletion of models such as Ticket, Queue, FollowUp, 
Attachment, PreSetReply, EmailTemplate, KBCategory, KBItem, and other related 
models. This script is auto-generated by South and defines the operations to 
apply these changes in the database schema (forwards) and to roll them back 
(backwards).

Modules used:
- south.db: Provides database abstraction layer for migrations.
- south.v2: Base migration class for custom migration logic.
- datetime: Used for default values for date fields.
"""
...
class Migration(SchemaMigration):
    """
    Handles schema changes for the Helpdesk application using South migrations.
    
    This migration creates and deletes tables and relationships for various
    helpdesk models including tickets, queues, attachments, followups, etc.
    """
    def forwards(self, orm):
        """
        Applies the schema changes to add Helpdesk-related models.

        Creates tables and relationships for:
        - Ticketing system (Ticket, FollowUp, TicketChange, etc.)
        - Knowledge base (KBCategory, KBItem)
        - Email integration (EmailTemplate, IgnoreEmail)
    ...
    def backwards(self, orm):
        """
        Reverts the schema changes made in `forwards()`.

        Deletes all Helpdesk-related tables and many-to-many relations
        including:
        - Ticket, Queue, FollowUp, Attachment, etc.
        - CustomField, TicketCustomFieldValue
        - PreSetReply, EscalationExclusion, and related M2M tables
        """
